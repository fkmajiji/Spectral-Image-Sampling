...
IMG_EXTENSIONS = ['.jpg', '.JPG', '.jpeg', '.JPEG', '.png', '.PNG', '.ppm', '.PPM', '.bmp', '.BMP', '.tif', '.bin', '.mat', 'h5']
...
def imread_uint(path, n_channels=3):
    ...
    return img
# --------------------------------------------
# get uint8 multispectral image of size HxWxn_channles
# --------------------------------------------
def load_img(filepath):
    ...
    return img
def save_msicube(img, img_path):
    tiff = TIFFimage(img.astype(np.uint8), description='')
    tiff.write_file(img_path,
                    compression='none')
    del tiff
# --------------------------------------------
# matlab's imwrite
...
def mask_input(GT_image, msfa_size):
    mask = np.zeros((GT_image.shape[0], GT_image.shape[1], msfa_size ** 2), dtype=GT_image.dtype)
...
def adaptive_masking(raw, msfa_size, pos_x, pos_y):
    mask = np.zeros((raw.shape[0], raw.shape[1]), dtype=raw.dtype)
    # for i in range(0, msfa_size):
    #     for j in range(0, msfa_size):
    #         mask[i::msfa_size, j::msfa_size, i * msfa_size + j] = 1
    mask[pos_x::msfa_size, pos_y::msfa_size] = 1
